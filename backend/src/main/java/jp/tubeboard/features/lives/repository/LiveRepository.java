...
public interface LiveRepository extends JpaRepository<Live, UUID> {
        ...
        List<Live> findAllByTenantIdOrderByCreatedAtDesc(UUID tenantId);
        ...
        @Query("SELECT l FROM Live l JOIN UserTenant ut ON ut.tenant = l.tenant "
                        + "WHERE l.tenant.id = :tenantId AND ut.user.id = :userId "
                        + "AND l.deletedAt IS NOT NULL AND l.deletedAt >= :cutoff AND ut.deletedAt IS NULL "
                        + "ORDER BY l.deletedAt DESC")
        List<Live> findAllTrashedByTenantIdAndAccessibleByUserId(@Param("tenantId") UUID tenantId,
                        @Param("userId") Long userId, @Param("cutoff") LocalDateTime cutoff);

        @Query("SELECT l FROM Live l JOIN UserTenant ut ON ut.tenant = l.tenant "
                        + "WHERE l.id = :id AND ut.user.id = :userId "
                        + "AND l.deletedAt IS NOT NULL AND ut.deletedAt IS NULL")
        Optional<Live> findTrashedByIdAndAccessibleByUserId(@Param("id") UUID id,
                        @Param("userId") Long userId);
}
